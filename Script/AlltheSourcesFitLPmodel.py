...
#plot fit function    
    ...
    fichier = open("out/"+source.replace(" ","")+'_forSimu3FGLPosition.xml','r+w')
    fichier2 = open("out/"+'tmp.xml','w')
    with  fichier as f:
        for line in f:
            
            if line.replace(" ","").replace("\n","") == '<sourcename="'+str(source).replace(" ","")+'"type="PointSource">':
                line = line.replace('type="PointSource">','type="PointSource" tscalc="1">')
                print line
            fichier2.write(line)
                #print "ok"
            #line[ite] = line
...
with open("Source4.txt") as f:
   for line in f:
        GetSourceInformation(line.split("\t")[0],"")
        print (line.split("\t")[0])
#norm,alpha,beta=GetSourceInformation("AP Librae","")
#norm,alpha,beta=GetSourceInformation("AP Librae","HESS_ApLIB_2015_loose.dat")
